...
Class: CS 777 - Fall 1
Date: Sep 2021
Homework  # Assignemnt5- Task 3 Logistic

So I am using np.random to select the features
...
#myRDD_modified = myRDD.map(lambda x: x[0]).zip(myRDD.map(lambda x: np.sort(x[1])))
myRDD_modified = myRDD.map(lambda x: x[0]).zip(myRDD.map(lambda x: np.random.choice(x[1], size=random_size, replace=False)))
myRDD_modified.cache()
...
### Implemented dimentionality reduction by using random

#myRDD_modified = myRDD.map(lambda x: x[0]).zip(myRDD.map(lambda x: np.sort(x[1])))
myRDD_test_data_modified = myRDD_test_data.map(lambda x: x[0]).zip(myRDD_test_data.map(lambda x: np.random.choice(x[1], size=random_size, replace=False)))
myRDD_test_data_modified.cache()

######################################################################################################################

### building with Label Points

my_data = myRDD_modified.map(lambda x: LabeledPoint(x[0], list(x[1])))
my_data_test = myRDD_test_data_modified.map(lambda x: LabeledPoint(x[0], list(x[1])))

end_read_data = time.time()

print (" -------Logistic Regression  total Read time in sec : ",end_read_data-start_read_data)

## Model training

start_model_training = time.time()

model = LogisticRegressionWithLBFGS.train(my_data,iterations=100)
...
print ("----------- Logistic Regression total Model testing and training time in sec : ",end_model_evaluation-start_model_training)
### lp[0]= Actual
### lp[1]= Prediected
...
print ("----------- Confusion matrix Logistic Regression calcuation time in sec : ",end_conf_matrix-start_conf_matrix)

total_end_time = time.time()

print (" ---- Logistic Regression total time in sec : ",total_end_time-start_read_data)
